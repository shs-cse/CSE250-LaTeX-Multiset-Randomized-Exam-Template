...
\def\minnumpagesperset{2} % A4=2, A3=4
...
\def\questionsintwocolumn{true}
\def\insertomrsheet{true}
\def\showsetinheader{true}
% --------------------------------PACKAGES------------------------------------------
\documentclass[addpoints, a4paper]{exam}
\printanswers % comment to hide answers
\usepackage[overload]{exam-randomizechoices} % useful options: norandomize, overload
\usepackage{anyfontsize, amssymb, amsmath, refcount}
...
% ---------------------------------PREAMBLE-----------------------------------------
\setlength{\columnseprule}{0.4pt} % for two columns
...
\newenvironment{cctikz}[1][\unskip]{\begin{center}\begin{tikzpicture}[#1]}{\end{tikzpicture}\end{center}}
...
\cs_new:Nn \set_questions:{
    ...
    \tl_if_eq:NnTF \questionsintwocolumn {true}    
    ...
    \endgradingrange{\set}
    \int_do_while:nn {\thepage<=\minnumpagesperset} {\null\clearpage}
}
\ExplSyntaxOff
